...
import { TimelineController } from "../../components/charts/timeline/timeline.controller";
...
const TimelinePageComponent: React.FC = () => {
    const ref = useRef<TimelineController<typeof data[number]> | null>(null);
    return <Page>
        <Page.Header />
        <Page.Body className={classes.timeline_page}>
            <div className={`${classes.section} ${classes.timeline}`}>
                <Timeline data={dataPoints} categories={categories} instanceRef={ref} />
            </div>
